 ...
 *订单下单
 **/
define('js/shop_pc/view/productPfManager', [
    ...
    function(ProductManagerTpl, ProductFormTpl, Cache, AlertUI, Header, ProductApi, CommunityApi, OrderApi, StringUtil) {
        ...
        var LayoutView = Backbone.View.extend({
            ...
            render: function() {
                ...
                //批发不需要的逻辑
                t.$el.find('.salesP').addClass('hidden');
                t.$el.find('.commossionP').addClass('hidden');
                t.$el.find('.multiSet').parent().addClass('hidden');
                t.$el.find('.resetPrice').parent().addClass('hidden');
                t.$el.find('.setCommission').parent().addClass('hidden');
                t.$el.find('.reSyncProduct').parent().addClass('hidden');
                ...
                var activityType = Cache.get('activityType'); //活动勾选
                if (activityType) {
                    var tmpDom = t.$el.find('.activity[type="' + activityType + '"]');
                    if (tmpDom && tmpDom.length > 0) {
                        tmpDom.addClass('active');
                    }
                }
            ...
            activity: function(e) {
                ...
                if (current.hasClass('active')) {
                    ...
                    t.$el.find('.activity').removeClass('active');
            ...
            loadData: function(isHideLackStock) {
                ...
                var activityTypeDom = t.$el.find('.activity.active');
                var activityType = '';
                if (activityTypeDom && activityTypeDom.length > 0) {
                    activityType = activityTypeDom.attr('type');
                }

                Cache.set('productName', productName); //缓存产品名称

                Cache.set('activityType', activityType); //缓存活动勾选
                ...
                if (list && list.length > 0) {
                    ...
                    for (var i = 0; i < list.length; i++) {
                        ...
                        var str = '<tr></tr>' +
                            ...
                            '<td class="width300 goDetail cp" style="width:500px;">' + product.name + '</td>' +
                            '<td style="width:100px;" class="' + (t.isNotManager ? "hidden" : "") + ' ' + (t.isNotShowBranchShopPrice ? "hidden" : "") + '">' + shopPrice + '</td>' +
                            '<td style="width:100px;">' + product.price + '</td>' +
                            '<td style="width:100px;" class="hidden">￥ <input type="text" ' + (t.isNotManager ? "readonly" : "") + ' class="priceSales p5" style="width:60px;border-radius: 8px;    border: 1px solid #d3def0;  ' + (t.isNotManager ? "border:0;" : "") + '" value="' + product.priceSales + '" price="' + product.priceSales + '"/></td>' +
                            '<td style="width:100px;" class="hidden">￥ <input type="text" ' + (t.isNotManager ? "readonly" : "") + ' class="commission p5" style="width:60px;border-radius: 8px;    border: 1px solid #d3def0; ' + (t.isNotManager ? "border:0;" : "") + '" value="' + product.commission + '" price="' + product.commission + '"/></td>' +
                            '<td style="width:100px;">' + t._getStockName(product.mount) + '</td>' +
                            '<td style="width:150px;" class="right-radius10"><a href="javascript:void(0)" class="a_btn btn_block quickOrderNew btn_radius quickOrder" productBarCode="' + product.productBarcode + '">快速进货</a></td>' +
                            '</tr>';
            ...
            loadCategory: function() {
                ...
                ProductApi.getProductCategoryList({}, function(data) {
                    ...
                    if (data && data.categoryList) {
                        var isFullcut = '';
                        var isDiscount = '';
                        isFullcut = data.isFullcut;
                        isDiscount = data.isDiscount;
                        //填充产品分类
                        t.fillProductCategory(data.categoryList, isFullcut, isDiscount);
                    }
                }, function(code, msg) {
                    tipsAlert.close();
                    if (code == -520) {
            ...
            //r如果有缓存  填充页面
            fillProductCategory: function(list, isFullcut, isDiscount) {
                var t = this;
                if (list && list.length > 0) {
                    ...
                    if (isFullcut) {
                        t.$el.find('.productCategory').append('<li class="activity  mj" type="mj"></li>');
                    }
                    if (isDiscount) {
                        t.$el.find('.productCategory').append('<li class="activity  zk" type="zk"></li>');
                    }
